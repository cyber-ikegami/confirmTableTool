...
const TableBuilder = (props: {
    ...
    // レコード
    const recordList = props.resourseManager.getRecordList(props.target, props.columnInfoList.map(info => info.columnName));
        ...
        if (filterList.length !== 0) {
            recordList.forEach((record) => {
                ...
                const isFilterList: boolean[] = record.map((cell, i) => {
                    ...
                    if (props.columnInfoList[i].convertList != undefined) {
                        cellValue = convert((props.columnInfoList[i].convertList) as ContentsUtil.ConvertInfo[], cell)?.value;
                    }
    ...
    for (let i = 0; result.length >= 100 ? i < 100 : i < result.length; i++) {
        cellJsxList = result[i].map((cell, j) => {
            ...
            let cellValue: any = cell;
            if (props.columnInfoList[j].convertList != undefined) {
                cellValue = convert((props.columnInfoList[j].convertList) as ContentsUtil.ConvertInfo[], cell)?.value;
            }
            return (
    ...
    return (
        <_Frame>
            <_ContentsFrame>
                ...
                <_Header ref={headerRef}>{headerJsxList}</_Header>
                <_Header ref={headerRef}>{filterJsxList}</_Header>
...
// 変換
const convert = (convertList: ContentsUtil.ConvertInfo[], cell: string) => {
